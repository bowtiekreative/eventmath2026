...
class EventMathCodeGen {
  ...
  generate(ast) {
    ...
    // Header
    this._line('// Generated by EventMath Compiler v0.2');
    ...
    if (this._varDecls.length > 0) {
      for (const decl of this._varDecls) {
        ...
        if (decl.value !== undefined) {
          this._line(`let ${safe} = ${decl.value};`);
        } else {
          this._line(`let ${safe};`);
        }
      }
      ...
      switch (stmt.type) {
        ...
        case 'Mark':
          if (!this._vars.has(stmt.name)) {
            ...
            this._varDecls.push({ name: stmt.name, value: val });
          }
          break;
        case 'Set':
          if (!this._vars.has(stmt.name)) {
